...
class TradingBot:
    ...
    async def show_settings(self, query):
        """Show settings menu with examples"""
        try:
            config = get_config()
            
            settings_text = "⚙️ Bot Settings\n\n"
            settings_text += "📋 Current Settings:\n"
            settings_text += f"📊 Trading Pair: {config.get('trading_pair', 'XRP_USDT')}\n"
            settings_text += f"💰 Position Size: {config.get('position_size', 0.1)}%\n"
            settings_text += f"🛑 Stop Loss: {config.get('stop_loss_percentage', 1.5)}%\n"
            settings_text += f"📈 Take Profit: {config.get('take_profit_percentage', 2.5)}%\n"
            settings_text += f"📊 RSI Period: {config.get('rsi', {}).get('period', 14)}\n\n"
            settings_text += "💡 Examples:\n"
            settings_text += "• Position Size 0.5% = $50 on $10,000 balance\n"
            settings_text += "• Stop Loss 1.5% = $7.50 loss on $500 trade\n"
            settings_text += "• Take Profit 2.5% = $12.50 profit on $500 trade\n"
            settings_text += "• RSI Period 14 = Standard sensitivity\n\n"
            settings_text += "Select setting to configure:"
            
            keyboard = [
                [InlineKeyboardButton("📊 Trading Pair", callback_data="config_trading_pair")],
                [InlineKeyboardButton("💰 Position Size", callback_data="config_position_size")],
